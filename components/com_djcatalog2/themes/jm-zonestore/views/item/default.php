...
$addtocart_class = ( (int)$this->item->available == 1 && $this->params->get('items_show_cart_button_item', '1') == 1 && ($this->params->get('cart_enabled', false) || $this->params->get('cart_query_enabled', 1)) ) ? 'addtocart' : '';

$app = JFactory::getApplication();
$template = $app->getTemplate(true);
$tparams = $template->params;
$accordion = ($tparams->get('productAcco', 0) == 1) ? true : false;
...
<div itemscope itemtype="http://schema.org/Product">
	...
	<div id="djcatalog" class="djc_clearfix djc_item<?php echo $this->params->get( 'pageclass_sfx' ).' djc_theme_'.$this->params->get('theme','default'); if ($this->item->featured == 1) echo ' featured_item'; ?>">
		...
		<h1 class="componentheading<?php echo $this->params->get( 'pageclass_sfx' ) ?> djc_pagdjc_cart_variantse_heading" <?php if (!$showItemTitle) { echo 'itemprop="name"'; } ?>>
		...
		<?php } ?>
			...
			if ($this->item->images && (int)$this->params->get('show_image_item', 1) > 0) {
				echo '<div class="djc_images_wrap">';
				echo $this->loadTemplate('images');
				if ( $this->item->featured == 1 ) { 
					echo '<span class="djc_featured_wrap"><img class="djc_featured_image" alt="'.JText::_('COM_DJCATALOG2_FEATURED_ITEM').'" title="'.JText::_('COM_DJCATALOG2_FEATURED_ITEM').'" src="'.DJCatalog2ThemeHelper::getThemeImage('featured.png').'" /></span>';
				}
				echo '</div>';
			} 
			...
			<div class="djc_top_wrap <?php echo $addtocart_class; ?>">
				...
				<h2 class="djc_title" itemprop="name">
					...
					<?php
					if ( !($this->item->images && (int)$this->params->get('show_image_item', 1) > 0) && $this->item->featured == 1 ) { 
				...
				<div class="djc_item_info">
					...
					<?php 
						if (! $accordion && count($this->attributes) > 0) {
									...
									<div class="accordion">
										...
										if (count($this->attributes) > 0) { ?>
											<div class="accordion-group">
												<div class="accordion-heading"><a href="#acco-attributes" data-toggle="collapse" class="accordion-toggle collapsed"><?php echo JText::_('COM_DJCATALOG2_SPECIFICATION')?></a></div>

												<div class="accordion-body collapse" id="acco-attributes"><div class="accordion-inner">

												<?php $grouppedAttributes = (bool) $this->params->get('group_attributes_item', '0') == '1';
												$attributes_body = ($grouppedAttributes) ? array() : '';
												foreach ($this->attributes as $attribute) {
													$this->attribute_cursor = $attribute;
													if ($grouppedAttributes) {
										...
										<?php //reviews
											if($this->item->event->afterDJCatalog2DisplayContent) { ?>
											<div class="accordion-group">

												<div class="accordion-heading"><a href="#acco-reviews" data-toggle="collapse" class="accordion-toggle collapsed"><?php echo JText::_('COM_DJCATALOG2_REVIEWS')?></a></div>

												<div class="accordion-body collapse" id="acco-reviews"><div class="accordion-inner">
				...
				<div class="djc_wrap_bottom">
					...
					$span_bottom_1 = ( ($this->params->get('show_files_item', 1) > 0 && ($this->item->files = DJCatalog2FileHelper::getFiles('item',$this->item->id))) || ( (int)$this->params->get('show_location_map_item', 1) > 0 || (int)$this->params->get('show_location_details_item', 1) > 0) ) ? 'span6' : 'span12';
					$span_bottom_2 = ( $span_bottom_1 == 'span6' ) ? 'span6' : 'span12';

					?>

					<div class="<?php echo $span_bottom_1; ?>">
					...
					<div class="<?php echo $span_bottom_2; ?>">
						<?php if(! $accordion && $this->item->event->afterDJCatalog2DisplayContent) { ?>
							<div class="djc_post_content">
								<?php echo $this->item->event->afterDJCatalog2DisplayContent; ?>
							</div>
						<?php } ?>
