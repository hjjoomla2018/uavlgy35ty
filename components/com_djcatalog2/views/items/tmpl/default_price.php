...
if ($prices['display'] != $prices['old_display'] ) {
	if ($this->params->get('show_old_price', '1') == '1') {
		echo ($list_layout!='table') ? '<span class="djc_price_label">'.JText::_('COM_DJCATALOG2_PRICE').': </span>' : '';
...
if ($prices['display2nd'] !== false && $show_tax) {
	echo '<span class="djc_price_without_tax">'.JText::sprintf('COM_DJCATALOG2_PRICE_WITHANDWITHOUT_TAX', DJCatalog2HtmlHelper::formatPrice($prices['display2nd'], $this->params, false), DJCatalog2HtmlHelper::formatPrice($prices['tax'], $this->params, false)).'</span>';
} else if ($show_tax) {
	echo '<span class="djc_price_with_tax">'.JText::sprintf('COM_DJCATALOG2_PRICE_WITH_TAX', DJCatalog2HtmlHelper::formatPrice($prices['tax'], $this->params, false)).'</span>';
}
