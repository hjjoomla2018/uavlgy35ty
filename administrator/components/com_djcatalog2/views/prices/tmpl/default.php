...
<form action="<?php echo JRoute::_('index.php?option=com_djcatalog2&view=prices');?>" method="post" name="adminForm" id="adminForm">
	...
	<table class="table table-striped">
		...
		<tfoot>
			<tr>
				<td colspan="<?php echo ($has_group) ? '8' : '7'; ?>">
		...
		<?php foreach ($this->items as $i => $item) :
			...
			<tr class="row<?php echo $i % 2; ?>">
				<td>
					<a target="_blank" href="<?php echo JRoute::_('index.php?option=com_djcatalog2&task=item.edit&id='.$item->id);?>"><?php echo $this->escape($item->name); ?></a>
					<p class="smallsub">
				...
				<td class="center">
					<div class="input-append">
						<input class="input input-mini inputbox djc_stock" type="text" value="<?php echo $item->stock; ?>" name="djc_prices[<?php echo $item->id ?>][stock]" />
							<select id="filter_cusomtergroup" name="djc_prices[<?php echo $item->id?>][onstock]" class="input input-small ">
								<?php 
								echo JHtml::_('select.options', $onstock, 'value', 'text', $item->onstock, true);?>
							</select>
					</div>
				</td>
				<td class="center">
