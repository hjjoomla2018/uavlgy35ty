...
class Djcatalog2ViewCartField extends JViewLegacy {
	...
	public function display($tpl = null)
	{
		...
		if ($this->fieldId > 0) {
			...
			$query->select('*')->from('#__djc2_cart_extra_fields')->where('id='.(int)$this->fieldId);
			$db->setQuery($query);
			$this->field = $db->loadObject();
			
			if ($this->field) {
				$query = $db->getQuery(true);
				$query->select('*')->from('#__djc2_cart_extra_fields_options')->where('field_id='.(int)$this->fieldId)->order('ordering ASC');
				$db->setQuery($query);
