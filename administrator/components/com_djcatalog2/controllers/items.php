	...
	public function set_prices() {
		...
		if (!empty($prices)) {
			foreach ($prices as $item_id => $price) {
				...
				if (isset($price['old']) && isset($price['new']) && floatval($price['new']) != floatval($price['old'])) {
					$update = true;
					$row->price = floatval($price['new']);
				}
				
				if (isset($price['stock']) && isset($price['onstock'])) {
					$update = true;
		...
		if (!empty($rows)) {
			foreach ($rows as $item) {
				if ($group  > 0) {
					...
					if ($price_id > 0) {
						if (!$db->updateObject('#__djc2_prices', $price_item, 'id', false)) {
							$app->enqueueMessage($db->getErrorMsg(), 'error');
						}
					} else {
						if (!$db->insertObject('#__djc2_prices', $price_item)) {
							$app->enqueueMessage($db->getErrorMsg(), 'error');
						}
					}
				} else {
					if (!$db->updateObject('#__djc2_items', $item, 'id', false)) {
						$app->enqueueMessage($db->getErrorMsg(), 'error');
					}
				}
			}
