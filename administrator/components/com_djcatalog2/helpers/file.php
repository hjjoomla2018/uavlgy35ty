...
class DJCatalog2FileHelper extends JObject {
		...
		if (count($files)) {
			...
			foreach($files as $k => &$file) {
				...
				if ($copy) {
					...
					$file->id = 0;
					$file->fullname = JString::strtolower(self::createFileName($file->fullname, $destination));
					...
					$newname = JString::strtolower(JString::substr($realname.'_'.$item->alias, 0, 200).'.'.self::getExtension($file->fullname));
					...
					if (!in_array($file->ext, $whitelist)) {
						$app->enqueueMessage(JText::sprintf('COM_DJCATALOG2_UPLOADER_EXT_ERROR', $realname), 'error');
						JLog::add(JText::_('COM_DJCATALOG2_UPLOADER_EXT_ERROR'), JLog::WARNING, 'jerror');
						continue;
					}
	...
	public static function getUploader($uploader_type, $record_type, $item_id, $limit, $size_limit, $whitelist, $files = array(), $multiple_upload) {
		...
		$attachment_groups = trim($params->get('attachment_groups', ''));
		...
		if (self::$plUpload_scripts_included == false) {
			...
			if (count($groups) > 0) {
				...
				$script_vars['attachment_groups'][] = '<option value="">'.'- '.JText::_('JNONE').' -'.'</option>';
