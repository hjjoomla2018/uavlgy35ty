...
abstract class ModJMArticlesCategoryHelper
{
	...
	public static function createThumbs($params, &$item, $force = false) {
		...
		if ( !empty($imgPath) && JFile::exists($imgPath) ) {
				...
				$creationMethod = $params->get('image_resizing', 2);
				...
				if ($creationMethod == 2) { //height auto (fit to width)
					...
					$dirname = $sourceType . '_w_' . $thumbWidth .'x' . $thumbHeight;
				} elseif($creationMethod == 3) { //width auto (fit to height)
					$targetWidth = $thumbHeight * $ratio;
					$thumbWidth = floor($targetWidth);
					$dirname = $sourceType . '_h_' . $thumbWidth .'x' . $thumbHeight;
				} elseif($creationMethod == 4) {
					$dirname = $sourceType . '_c_' . $thumbWidth .'x' . $thumbHeight;
				} else {
					$dirname = $sourceType . '_r_' . $thumbWidth .'x' . $thumbHeight;
				}
				...
				$thumbFileName = $thumbsFolder . DIRECTORY_SEPARATOR . $filename;
