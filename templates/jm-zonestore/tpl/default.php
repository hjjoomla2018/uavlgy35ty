...
//maincontent width
$maincontentmenuitem = in_array($app->input->get('Itemid'), (array)$this->params->get('mainContentMenuItem', array()), true);
$maincontentclass = ($maincontentmenuitem) ? 'maincontent_width' : '';

//fixed column width
$fixedcolumnmenuitem = in_array($app->input->get('Itemid'), (array)$this->params->get('fixedColumnMenuItem', array()), true);
$fixedcolumnclass = ($fixedcolumnmenuitem) ? 'fixedcolumn_width' : '';
...
$blocks = $this->getBlocks('header,top1,top2,system-message,main,bottom1,bottom2,footer-mod,footer','comingsoon');
?>
<!DOCTYPE html>
...
<body class="<?php echo $responsivedisabled.' '.$offcanvasposition. ' '.$maincontentclass. ' '.$fixedcolumnclass.' '.$contrast. ' '.$width.' '.$fsize. ' '.$wcagsettings; ?>">
  <div id="jm-allpage">
  	<?php
	if(($comingsoon!='0') AND (!empty($comingsoondate)) AND ($futuredate=='1') AND JFactory::getApplication()-> isSite() AND JFactory::getUser()->guest) {
  		...
  		<?php endif; ?>
		<?php
		$this->renderBlock('top-bar');
		$this->renderBlock('logo-nav');
		?>
		<div id="jm-wrapper">
